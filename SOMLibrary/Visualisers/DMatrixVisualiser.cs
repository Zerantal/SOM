...
{
    /*
    [SOMPluginDetail("DMatrix Renderer", "Renders map as a distance matrix")]
    public class DMatrixVisualiser : IVisualiser
    {
        private IDrawer m_Drawer;
        private String errMsg;

        public DMatrixVisualiser()
        {
        }
        ...
        public void VisualiseMap(ISOM algorithm)
        {
            ...
            int[] DMatColour = new int[map.XDim * map.YDim];
            int i, j;

            // Create UMatColour values
            double max = double.MinValue;
            double min = double.MaxValue;
            for (i = 1; i <= U.Rows; i += 2)
                for (j = 1; j <= U.Columns; j+=2)
                {                                       
                    if (U[i, j] > max)
                        max = U[i, j];
                    if (U[i, j] < min)
            ...
            for (i = 1; i <= U.Rows; i+=2)
                for (j = 1; j <= U.Columns; j+=2)
                {
                    ...
                    DMatColour[idx] = 255 << 24 | c << 16 | c << 8 | c;
                    idx++;
                }
            ...
            {
                float tmpW = 2f * (map.XDim + 1) * 0.866f;
                float tmpH = (3 * map.YDim + 1) / 2;
                ...
                for (j = 0; j < map.YDim; j++)
                {
                    y = ((3f / 2f) * j + 1) * h;
                    x = w / 2;
                    if ((j + 1) % 2 == 0)
                        x += w / 2;
                    for (i = 0; i < map.XDim; i++)
                    {
                        ...
                        m_Drawer.FillPolygon(cell, unchecked((int)DMatColour[idx]));
                        idx++;
                        x += w;
                    }
                }
                ...
                for (j = 0; j < map.YDim; j++)
                    for (i = 0; i < map.XDim; i++)
                    {
                        m_Drawer.FillRectangle(new Point((float)(i * width), (float)(j * width)),
                            width, width, unchecked((int)DMatColour[idx]));
                        idx++;
                    }
            }
        }
